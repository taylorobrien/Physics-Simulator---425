...
void BaseApplication::createGUI(){
	...
	Label* gameInstruc = mTrayMgr->createLabel(TL_TOPRIGHT, "Instructions", "Throw the fish in the top of the barrel!", 400);

	Ogre::StringVector items;
	items.push_back("Score");
	items.push_back("Fish Remaining");
	items.push_back("Mass");
	items.push_back("Speed");
	items.push_back("Angle");
	mParamsPanel = mTrayMgr->createParamsPanel(OgreBites::TL_BOTTOMRIGHT,"SamplePanel",200,items);
	mTrayMgr->showAll();

	restartButton = mTrayMgr->createButton(TL_TOPLEFT, "MyButton", "Game Over Click To Play Again", 250.0);
	restartButton->hide();
